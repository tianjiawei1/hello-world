...
SpringBoot来简化Spring应用开发，约定大于配置，去繁从简，just run就能创建一个独立的产品级别的应用
...
### 3.环境配置
	...
	**推荐**jdk1.8，maven3.x，idea 2017， SpringBoot 1.5.9
...
spring-boot-starter：spring-boot场景启动器；帮我们导入了web模块正常运行所依赖的组件(如Tomcat，starter)；
...
pringFactoriesLoader.loadFactoryNames(EnableAutoConfiguration.class,classLoader)；
...
#### 6.1使用 Spring Initializer快速创建项目
...
```
 ...
 1响应页面；去哪个页面是由**DefaultErrorViewResolver**解析得到的；
